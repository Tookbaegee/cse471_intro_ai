...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        "*** YOUR CODE HERE ***"
        cpValues = util.Counter()
        updated = util.Counter()
        for _ in range(self.iterations):
            
            for state in self.mdp.getStates():
               
               #get best action and calculate the new q value for the state.
                bestAction = self.computeActionFromValues(state)
                if bestAction:
                    cpValues[state] = self.computeQValueFromValues(state, bestAction)
                    updated[state] = 1
                
            for state in self.mdp.getStates():
                #apply to the real qValues of states if q value has been updated during iteration.
                if updated[state]:
                    self.values[state] = cpValues[state]
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextstate, prob in nextstatesAndProbs:
            #apply the equation for q value of a state for action transition.
    ...
    def computeActionFromValues(self, state):
        ...
        #get the key for the maximum q value for every possible action from a state.
